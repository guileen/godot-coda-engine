...
如果只想检查 CODA 的运行效果，不需要打开脚本或终端。在本项目文件夹中（就是能看到本 README 和 `project.godot` 的文件夹），双击：

1. 双击 [体验 CODA 事件编辑器.command](<体验 CODA 事件编辑器.command>)，打开事件编辑器；右侧选择 **CODA 事件** 面板中的“应用奖励”流程。
2. 需要直接看运行效果时，关闭或最小化编辑器，再双击 [体验 奖励流程.command](<体验 奖励流程.command>)。
3. 出现“领取关卡奖励”画面后，点击“领取奖励 +25”。观察积分从 100 动画增加到 125、上一条奖励被替换，并出现结算通知。再点击“验证条件：奖励为 0”，确认条件不成立时积分和奖励条目不变。“重置样例”可重新体验。

也可以双击 [体验 具身演示.command](<体验 具身演示.command>)，直接观察机器人头部动作、表情、中断和拒绝外部占用时的反馈。

正式用户观察时由主持人提前完成前 3 步，参与者从已打开的奖励界面开始；参与者只判断表达是否自然、是否理解流程、能否顺利修改和预览，以及运行结果是否符合预期。
...
从修改 [`examples/reward-claim.coda`](examples/reward-claim.coda) 里的奖励流程开始，再运行一次 `text-check`。生成文件位于隐藏的 `.coda/generated/` 兼容目录；它们是输出，不应手动修改。
...
- [CODA 用户使用手册](docs/zh-CN/user-manual.md) / [试玩者与观察者体验说明](docs/demos/体验说明.md)

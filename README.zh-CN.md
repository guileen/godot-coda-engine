...
如果只想检查 CODA 的运行效果，不需要打开脚本或终端：

1. 打开 Godot 4.7.2 的项目管理器。
2. 选择“导入”，选中本项目文件夹（里面能看到 `project.godot`），再点击“导入并编辑”。
3. 在 Godot 编辑器右上角点击“运行项目”（三角形按钮，快捷键 F5）。
4. 出现“领取关卡奖励”画面后，点击“领取奖励 +25”。观察积分从 100 动画增加到 125、上一条奖励被替换，并出现结算通知。再点击“验证条件：奖励为 0”，确认条件不成立时不会改积分或奖励条目。“重置样例”可重新体验。

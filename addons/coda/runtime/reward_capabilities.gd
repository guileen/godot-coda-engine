...
func read(args: Dictionary) -> Variant:
	...
	if not is_instance_valid(target) or not target.has_meta("_coda_fields"):
		return null
	var fields: Dictionary = target.get_meta("_coda_fields", {})
...
func animate_number(args: Dictionary) -> Variant:
	...
	tween.tween_method(func(value: float):
		...
		target.set_meta("_coda_fields", fields)

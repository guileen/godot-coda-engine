...
func _test_crash_at(point: String) -> void:
	...
	if crash_point.is_empty(): crash_point = OS.get_environment("CODA_TEST_CRASH_AT")
	...
	if marker_path.is_empty(): marker_path = OS.get_environment("CODA_TEST_CRASH_MARKER")

...
4. Review the reward example as a user task: identify the trigger, understand the condition, see the reward change, and recognize completion. The sample explains trigger → guard → score calculation/wait → row update → feedback, highlights completed/current steps as it runs, and marks the failed condition and skipped steps for a zero reward. Ask participants about clarity and naturalness, not source files or internal implementation.
...
## Narrow-dock insertion design profile

- **User and result:** A gameplay author unfamiliar with CODA can add a step exactly where intended, understand what to fill next, and know that cancelling leaves the saved flow untouched.
- **Surface and constraints:** A dense, stateful Godot Editor right dock. Inherit Godot's native control theme; this change adjusts task order and action width, not the visual brand.
- **Reference prior:** Borrow YAMI Event Window's command search and selected-point insertion. Do not copy its implementation, skin, or assets.
- **Decisions:** Keep only “新建事件” and “更多操作” in the primary toolbar, size the menu to its label, default insertion to after the selected step, expose before/after/end and condition-branch positions, and explain the required event selection when none is active. Keep new steps as drafts until the user confirms.
- **Trade-off:** Showing before/after choices adds a small amount of density, but only after a step is selected; it prevents a silent append to the wrong place. With no selected step, the predictable default is the flow end.
- **Evidence and limit:** Godot smoke will verify location choices, nested-branch placement, unselected-event guidance, and unchanged saved assets on cancellation. This cannot establish natural wording or discoverability; those still require observing target users in the visible editor.

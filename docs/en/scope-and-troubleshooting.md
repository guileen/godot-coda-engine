...
Check that the file is a `.coda.json` asset under `coda/events/`, that the EventAsset validates, and that the `CODA Event Editor` plugin is enabled. The dock rebuilds from the asset; its summary text is not a parseable source.

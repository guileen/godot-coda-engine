...
| --- | --- | --- |
| CODA stores a versioned `EventAsset@1` with stable IDs and preserves unknown fields. | `packages/local-core/src/coda/asset.js`, `contracts/gseos/event-asset.schema.json`, and the asset tests. | `npm test` |
| The generation path is `EventAsset → ExecutionPlan → GDScript`, with source maps and deterministic output. | `packages/local-core/src/coda/planner.js`, `packages/local-core/src/coda/codegen.js`, `tests/golden/ui.reward.apply.source-map.json`, and the checked-in reward fixture. | `npm run coda -- validate gseos/events/ui.reward.apply.gse.json` then `npm run coda -- generate gseos/events/ui.reward.apply.gse.json` |

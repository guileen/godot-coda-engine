...
| The generation path is `EventAsset → ExecutionPlan → GDScript`, with source maps and deterministic output. | `packages/local-core/src/coda/planner.js`, `packages/local-core/src/coda/codegen.js`, `tests/golden/ui.reward.apply.source-map.json`, and the checked-in reward fixture. | `npm run coda -- validate coda/events/ui.reward.apply.gse.json` then `npm run coda -- generate coda/events/ui.reward.apply.gse.json` |

...
export function formatGse(textOrAsset, mode = "preserve") {
  ...
  const asset = textOrAsset; const chinese = mode === "zh"; const keyword = chinese ? { if: "若", let: "令", do: "执行", await: "等待", intent: "意图", publish: "发出" } : { if: "if", let: "let", do: "do", await: "await", intent: "intent", publish: "publish" }; const lines = [chinese ? `事件 ${asset.event_id}：` : `event ${asset.event_id}:`];
  const walk = (nodes, indent) => { for (const node of nodes ?? []) { const pad = "  ".repeat(indent); if (node.command_id === "if") { lines.push(`${pad}${keyword.if}${chinese ? "（" : " ("}${formatExpression(node.params.condition)}${chinese ? "）：" : "):"}`); walk(node.children?.then, indent + 1); } else if (node.command_id === "let") lines.push(`${pad}${keyword.let} ${node.params.name} ${chinese ? "为" : "="} ${formatExpression(node.params.value)}`); else if (["do", "await"].includes(node.command_id)) lines.push(`${pad}${keyword[node.command_id]} ${node.params.capability}(${formatArguments(node.params.args)})`); else if (node.command_id === "motion_intent") lines.push(`${pad}${keyword.intent} ${node.params.intent}(${formatArguments(node.params.args)})`); else if (node.command_id === "publish") lines.push(`${pad}${keyword.publish} ${node.params.topic}(${formatExpression(node.params.payload)})`); } }; walk(asset.root, 1); return `${lines.join("\n")}\n`;
}
...
function formatArguments(args) { return Object.entries(args ?? {}).map(([key, value]) => `${key}: ${formatExpression(value)}`).join(", "); }

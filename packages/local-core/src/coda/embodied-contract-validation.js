...
const push = (items, code, message, path) => items.push(codaDiagnostic(code, message, { path }));
...
export function validateEmbodimentUnitRegistry(registry) {
  const diagnostics = [];
  if (!headerOk(registry, "EmbodimentUnitRegistry")) return codaReceipt([codaDiagnostic("INVALID_EMBODIMENT_UNIT_REGISTRY", "必须使用 EmbodimentUnitRegistry@1。")]);
...
export function validateReferenceFrameRegistry(registry) {
  const diagnostics = [];
  if (!headerOk(registry, "ReferenceFrameRegistry")) return codaReceipt([codaDiagnostic("INVALID_REFERENCE_FRAME_REGISTRY", "必须使用 ReferenceFrameRegistry@1。")]);
...
export function validateObservationContract(contract, { known_guard_refs = [] } = {}) {
  const diagnostics = [];
  if (!headerOk(contract, "ObservationContract")) return codaReceipt([codaDiagnostic("INVALID_OBSERVATION_CONTRACT", "ObservationContract 类型或版本无效。")]);
...
export function validateHybridModeGraph(graph, { known_guard_refs = [] } = {}) {
  const diagnostics = [];
  if (!headerOk(graph, "HybridModeGraph")) return codaReceipt([codaDiagnostic("INVALID_HYBRID_MODE_GRAPH", "HybridModeGraph 类型或版本无效。")]);
...
export function validateTrackingEnvelope(envelope) {
  const diagnostics = [];
  if (!headerOk(envelope, "TrackingEnvelope")) return codaReceipt([codaDiagnostic("INVALID_TRACKING_ENVELOPE", "TrackingEnvelope 类型或版本无效。")]);
...
export function validateControlContract(contract) {
  const diagnostics = [];
  if (!headerOk(contract, "ControlContract")) return codaReceipt([codaDiagnostic("INVALID_CONTROL_CONTRACT", "ControlContract 类型或版本无效。")]);
...
export function validateContinuationContract(contract) {
  const diagnostics = [];
  if (!headerOk(contract, "ContinuationContract")) return codaReceipt([codaDiagnostic("INVALID_CONTINUATION_CONTRACT", "ContinuationContract 类型或版本无效。")]);
...
export function validateTemporalCommandContract(contract) {
  const diagnostics = [];
  if (!headerOk(contract, "TemporalCommandContract")) return codaReceipt([codaDiagnostic("INVALID_TEMPORAL_COMMAND_CONTRACT", "TemporalCommandContract 类型或版本无效。")]);
...
export function validateSafetyAuthorityReceipt(receipt) {
  ...
  if (!receipt || typeof receipt !== "object" || receipt.receipt_type !== "SafetyAuthorityReceipt" || receipt.schema_version !== 1) return codaReceipt([codaDiagnostic("INVALID_SAFETY_AUTHORITY_RECEIPT", "消息必须使用 SafetyAuthorityReceipt@1。")]);
...
export function validateAuthorityClaimMatrix(matrix) {
  const diagnostics = [];
  if (!headerOk(matrix, "AuthorityClaimMatrix")) return codaReceipt([codaDiagnostic("INVALID_AUTHORITY_CLAIM_MATRIX", "AuthorityClaimMatrix 类型或版本无效。")]);
...
export function validateEmbodimentProtocolMessage(message, { now_tick = null, clock_domain = null, minimum_epoch = null, minimum_sequence = null, unit_registry = null, frame_registry = null } = {}) {
  ...
  if (!message || typeof message !== "object" || message.protocol !== "EmbodimentProtocol" || message.schema_version !== 1) return codaReceipt([codaDiagnostic("INVALID_EMBODIMENT_PROTOCOL_HEADER", "消息必须使用 EmbodimentProtocol@1。")]);
  ...
  const leaseRequired = ["reference", "mode_request", "authority_lease", "handoff", "barrier_receipt", "start_receipt", "transition_receipt", "terminal_receipt"].includes(message.message_type);

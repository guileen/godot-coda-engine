...
function checkFields(value, allowed, path, diagnostics) {
  ...
  for (const key of Object.keys(value)) {
    ...
    if (FORBIDDEN_PERSONAL_FIELDS.has(normalized)) {
      diagnostics.push(gseosDiagnostic("USER_OBSERVATION_PII_FIELD", `观察记录不得包含个人或项目身份字段：${key}。`, { path: `${path}/${key}` }));
    } else if (!allowed.has(key)) {
      diagnostics.push(gseosDiagnostic("UNDECLARED_USER_OBSERVATION_FIELD", `观察记录包含 schema 未声明的字段：${key}。`, { path: `${path}/${key}` }));
    }
  }
}
...
function checkPrivateText(value, path, diagnostics) {
  ...
  if (value.length > 240) diagnostics.push(gseosDiagnostic("USER_OBSERVATION_TEXT_TOO_LONG", "自由文本只能保留不超过 240 字的脱敏摘要，不得粘贴原始访谈记录。", { path }));
  if (PRIVATE_TEXT_PATTERNS.some((pattern) => pattern.test(value))) diagnostics.push(gseosDiagnostic("USER_OBSERVATION_PRIVATE_TEXT", "自由文本疑似包含邮箱、个人路径或 GitHub 身份链接，请先脱敏。", { path }));
}
...
export function validateUserObservationReport(report) {
  ...
  for (const [index, observation] of observations.entries()) {
    ...
    if (observation.target_user !== true) diagnostics.push(gseosDiagnostic("USER_OBSERVATION_NOT_TARGET_USER", "只能将目标用户观察计入 G-P3-U。", { path: `${path}/target_user` }));
    if (!["none", "implementation", "review", "maintenance"].includes(observation.implementation_involvement)) diagnostics.push(gseosDiagnostic("INVALID_USER_OBSERVATION_RELATIONSHIP", "必须明确参与者与实现项目的关系。", { path: `${path}/implementation_involvement` }));
    else if (observation.implementation_involvement !== "none") diagnostics.push(gseosDiagnostic("USER_OBSERVATION_NOT_INDEPENDENT", "实现者、审阅者或维护者不得计入独立用户样本。", { path: `${path}/implementation_involvement` }));
    if (observation.eligibility_verified_by_observer !== true) diagnostics.push(gseosDiagnostic("USER_OBSERVATION_ELIGIBILITY_NOT_CONFIRMED", "必须由观察主持人确认目标用户资格与未参与实现，不能只依赖样本自我声明。", { path: `${path}/eligibility_verified_by_observer` }));
    if (typeof observation.role_profile !== "string" || observation.role_profile.length === 0 || observation.role_profile.length > 80) diagnostics.push(gseosDiagnostic("INVALID_USER_OBSERVATION_ROLE_PROFILE", "role_profile 必须是最多 80 字的脱敏角色/经验摘要。", { path: `${path}/role_profile` }));
    else checkPrivateText(observation.role_profile, `${path}/role_profile`, diagnostics);
    if (!observation.consent || typeof observation.consent !== "object" || Array.isArray(observation.consent)) diagnostics.push(gseosDiagnostic("INVALID_USER_OBSERVATION_CONSENT", "consent 必须是结构化同意记录。", { path: `${path}/consent` }));
    ...
    if (typeof observation.consent?.recording_allowed !== "boolean") diagnostics.push(gseosDiagnostic("INVALID_USER_OBSERVATION_RECORDING_CONSENT", "必须明确记录是否允许录音/录像；默认应为 false。", { path: `${path}/consent/recording_allowed` }));
    ...
    if (!["pass", "fail", "blocked"].includes(observation.outcome)) diagnostics.push(gseosDiagnostic("INVALID_USER_OBSERVATION_OUTCOME", "outcome 必须是 pass/fail/blocked 之一。", { path: `${path}/outcome` }));
    if (!observation.tasks || typeof observation.tasks !== "object" || Array.isArray(observation.tasks)) diagnostics.push(gseosDiagnostic("INVALID_USER_OBSERVATION_TASKS", "tasks 必须是结构化任务结果。", { path: `${path}/tasks` }));
    ...
    for (const [summaryIndex, summary] of (Array.isArray(observation.quote_summaries) ? observation.quote_summaries : []).entries()) {
      if (typeof summary !== "string") diagnostics.push(gseosDiagnostic("INVALID_USER_OBSERVATION_SUMMARY", "quote_summaries 每项都必须是脱敏文本摘要。", { path: `${path}/quote_summaries/${summaryIndex}` }));
      else checkPrivateText(summary, `${path}/quote_summaries/${summaryIndex}`, diagnostics);
    }
    if (observation.blockers !== undefined && !Array.isArray(observation.blockers)) diagnostics.push(gseosDiagnostic("INVALID_USER_OBSERVATION_BLOCKERS", "blockers 必须是结构化阻塞摘要数组。", { path: `${path}/blockers` }));
  ...
  if (qualified.length < 3) diagnostics.push(gseosDiagnostic("USER_OBSERVATION_COUNT", "G-P3-U 至少需要 3 名未参与实现的目标用户。", { path: "/observations", actual: qualified.length, minimum: 3 }));

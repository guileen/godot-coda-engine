...
export function parseGse(text) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (ifMatch) { node = { node_id: nodeId, command_id: "if", params: { condition: expressionFromText(ifMatch[1]) }, children: { then: [], else: [] }, source_span: span }; parent.push(node); stack.push({ indent, nodes: node.children.then }); continue; }

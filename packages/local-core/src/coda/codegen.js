...
function emitInstruction(instruction, lines, map, indent = "  ") {
  ...
  lines.push(`${indent}# coda:event=${ref.event_id} node=${ref.node_id}`);
  ...
  switch (instruction.opcode) {
    ...
    case "Escape": lines.push(`${indent}# coda:escape inputs=${JSON.stringify(instruction.inputs)} outputs=${JSON.stringify(instruction.outputs)}`); lines.push(`${indent}${instruction.code}`); break;
...
export function generateGdscript(plan) {
  const fingerprint = planFingerprint(plan); const map = []; const lines = [`# GENERATED BY CODA. event=${plan.event_id} plan=${fingerprint}`, "# DO NOT EDIT. source is managed outside the runtime.", `extends RefCounted`, "", `func run(runtime, args: Dictionary, owner: Node) -> Variant:`, `  var ctx: CODA_RunContext = runtime.create_context(args, owner, ${JSON.stringify(plan.event_id)}, ${JSON.stringify(fingerprint)})`];
...
export function verifyManagedArtifact(source, expected) {
  ...
  if (!/^# GENERATED BY (?:CODA|GSEOS)\. event=/m.test(String(source))) diagnostics.push(codaDiagnostic("UNMANAGED_ARTIFACT", "工件缺少 CODA 生成标记（旧 CODA 标记仅作只读识别，避免覆盖既有生成物）。", { path: "/" }));
  if (!String(source).includes(marker)) diagnostics.push(codaDiagnostic("MANAGED_ARTIFACT_CHANGED", "受管工件的事件或计划指纹已变化；停止覆盖并要求显式接管。", { path: "/" }));

...
[[ -f "$GUI_PROJECT/coda/events/new-event.coda.json" ]]
DISPLAY_NAME="$(node -e 'console.log(require(process.argv[1]).display_name)' "$GUI_PROJECT/coda/events/new-event.coda.json")"
...
ROOT_COUNT="$(node -e 'console.log(require(process.argv[1]).root.length)' "$GUI_PROJECT/coda/events/new-event.coda.json")"
[[ "$ROOT_COUNT" == "1" ]]

run_input click 850 313
run_input click 1205 150
sleep 1
ROOT_COUNT_AFTER_DELETE="$(node -e 'console.log(require(process.argv[1]).root.length)' "$GUI_PROJECT/coda/events/new-event.coda.json")"

...
static void write_report(void) {
    ...
    if (path == NULL || *path == '\0') path = getenv("GSEOS_MALLOC_REPORT");
...
__attribute__((destructor)) static void gseos_malloc_report(void) { write_report(); }

__attribute__((used)) static struct {
    const void *replacement;
    const void *replacee;
} gseos_interposers[] __attribute__((section("__DATA,__interpose"))) = {
    {(const void *)gseos_malloc, (const void *)malloc},
    {(const void *)gseos_calloc, (const void *)calloc},
    {(const void *)gseos_realloc, (const void *)realloc},
    {(const void *)gseos_free, (const void *)free},
};

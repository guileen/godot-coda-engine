...
func _start() -> void:
	...
	var handle := CODA_RunHandle.new(1)
	...
	if handle.status != CODA_RunHandle.Status.CANCELLED:
		failures.append("cancel did not remain terminal")

	# Timeout is also a single terminal transition.
	var timeout_wait := CODA_WaitRegistration.new()
	...
	var owner_handle := CODA_RunHandle.new(2)
	owner_handle.status = CODA_RunHandle.Status.WAITING
	var owner_wait := CODA_WaitRegistration.new()
	var owner_id := owner.get_instance_id()
	owner_wait.settled.connect(func(_result: Dictionary):
		if is_instance_valid(instance_from_id(owner_id)):
			owner_handle.finish(CODA_RunHandle.Status.COMPLETED)
		else:
	...
	if owner_handle.status != CODA_RunHandle.Status.CANCELLED:
